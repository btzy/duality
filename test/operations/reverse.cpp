...
TEST_CASE("reverse operation", "[operation reverse]") {
    ...
    SECTION("from view with function call syntax") {
        view_assert_finite_random_access(operations::reverse(v), {5, 4, 3, 2, 1});
        view_assert_finite_random_access(v, {5, 4, 3, 2, 1});
        CHECK_THAT(vec, Catch::Matchers::RangeEquals({5, 4, 3, 2, 1}));
    }
    SECTION("from view without adaptor") {
        view_assert_finite_random_access(v | operations::reverse, {5, 4, 3, 2, 1});
        view_assert_finite_random_access(v, {5, 4, 3, 2, 1});
        CHECK_THAT(vec, Catch::Matchers::RangeEquals({5, 4, 3, 2, 1}));
    }
    ...
    SECTION("from vector without adaptor") {
        view_assert_finite_random_access(vec | operations::reverse, {5, 4, 3, 2, 1});
        view_assert_finite_random_access(v, {5, 4, 3, 2, 1});
        CHECK_THAT(vec, Catch::Matchers::RangeEquals({5, 4, 3, 2, 1}));
    }
